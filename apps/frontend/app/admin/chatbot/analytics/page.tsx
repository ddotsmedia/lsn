...
function Stat({ label, value, tone }: { label: string; value: string; tone?: string }) {
  return (
    <div className="rounded-xl border border-zinc-800/50 bg-[#0c0c14] p-4">
      ...
      <p className={`mt-1 text-2xl font-semibold ${tone ?? 'text-zinc-100'}`}>{value}</p>
    </div>
  );
}
    ...
    <ul className="space-y-2">
      {rows.map((row) => (
        <li key={row.label} className="flex items-center gap-3">
          <span className="w-28 shrink-0 truncate text-xs text-zinc-400">{row.label}</span>
          ...
          <span className="w-8 shrink-0 text-right text-xs text-zinc-400">{row.count}</span>
        </li>
      ))}
    </ul>
  );
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-zinc-100">Chatbot analytics</h1>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <section className="rounded-xl border border-zinc-800/50 bg-[#0c0c14] p-4">
            ...
            <section className="rounded-xl border border-zinc-800/50 bg-[#0c0c14] p-4 lg:col-span-2">
              ...
              {data.topUnanswered.length === 0 ? (
                <p className="text-sm text-zinc-500">Nothing unanswered in this period.</p>
              ) : (
                <ul className="divide-y divide-zinc-800/50">
                  {data.topUnanswered.map((q) => (
                    <li key={q.question} className="flex items-center justify-between gap-4 py-2">
                      <span className="truncate text-sm text-zinc-300">{q.question}</span>
